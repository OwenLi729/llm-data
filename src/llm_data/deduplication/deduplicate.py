...
def _large_star(e):
    und = e.select("u", "v").union_all(e.select(col("v").alias("u"), col("u").alias("v")))
    m = und.groupby("u").agg(col("v").min().alias("mn"))
    m = m.select("u", when(col("u") < col("mn"), col("u")).otherwise(col("mn")).alias("m"))
    return (
        und.join(m, on="u")
        .where(col("v") > col("u"))
...
def _small_star(e):
    d = _canon(e).select(col("v").alias("u"), col("u").alias("v"))  # u = larger endpoint
    j = d.join(d.groupby("u").agg(col("v").min().alias("m")), on="u").collect()
    return (
        j.select(col("v").alias("u"), col("m").alias("v"))
        .union_all(j.select("u", col("m").alias("v")))  # keep u attached to m (paper's N(u) ∪ {u})
        .where(col("u") != col("v"))
        .distinct()
        .collect()
    )
...
def fuzzy_dedupe(input_path, work, output_dir, key="doc_id", text="paragraph_text",
           num_hashes=112, ngram=5, seed=42, b=14, r=8):
    assert b * r == num_hashes
    daft.set_runner_ray()
    sigs, edges_dir, removal_dir = f"{work}/sigs", f"{work}/edges", f"{work}/removal"

    (
        daft.read_parquet(input_path, file_path_column="source_path",
                          checkpoint=daft.CheckpointConfig(daft.CheckpointStore(f"{work}/ckpt_sig"), on="source_path"))
        .select(col(key), col(text)
                .normalize(remove_punct=True, lowercase=True, nfd_unicode=True, white_space=True)
                .minhash(num_hashes=num_hashes, ngram_size=ngram, seed=seed, hash_function="xxhash")
                .alias("sig"))
    ...
    if not os.path.isdir(edges_dir):
        bands = (daft.read_parquet(sigs)
                 .select(col(key), band_keys(col("sig"), b, r).alias("bk"))
                 .explode("bk"))
        grouped = (bands.groupby("bk")
