...
def download_file(identifier, fileinfo, out_dir, retries=3):
    ...
    for attempt in range(retries):
        try:
            ...
            with urllib.request.urlopen(req, timeout=60) as response, open(tmp_dest, 'wb') as out:
                while True:
...
def main():
    ...
    parser.add_argument('--only', default=None,
                        help='comma-delimited list of sites to restrict to')
    ...
    if args.only:
        ...
        files = [f for f in files 
                 if os.path.basename(f["name"]).split('.')[0]
                 in wanted]
